...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        bcg_synthesis_kwargs["channel_base"] = \
            bcg_synthesis_kwargs["channel_base"] // 2
        self.z_dim = z_dim
        self.c_dim = c_dim
        self.w_dim = w_dim
        ...
        self.backbone = StyleGAN2Backbone(
            z_dim, c_dim, w_dim,
            img_resolution=rendering_kwargs['triplane_size'],
            img_channels=32*3*rendering_kwargs['triplane_depth'],
        ...
        self.decoder = OSGDecoder(
            32,
            {'decoder_lr_mul': rendering_kwargs.get(
                'decoder_lr_mul', 1),
             'decoder_output_dim': 32,
             'decoder_activation': rendering_kwargs['decoder_activation']})
        self.torgb = ToRGBLayer(32, 3, w_dim) if rendering_kwargs.get(
            'use_torgb_raw', False) else None
        self.bcg_synthesis = SynthesisNetwork(
            w_dim, img_resolution=self.superresolution.input_resolution,
            img_channels=32,
            **bcg_synthesis_kwargs) if \
            rendering_kwargs.get('use_background', False) else None
        # New mapping network for self-adaptive camera pose, dim = 3
        self.t_mapping = MappingNetwork(
            z_dim=z_dim, c_dim=c_dim, w_dim=3, num_ws=1,
            last_activation='linear', lr_multiplier=1.0, **mapping_kwargs)
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs

        self._last_planes = None

    def mapping(self, z, c, truncation_psi=1,
                truncation_cutoff=None, update_emas=False):
        if self.rendering_kwargs['c_gen_conditioning_zero']:
            c = torch.zeros_like(c)
        return self.backbone.mapping(
            z, c * self.rendering_kwargs.get('c_scale', 0),
    ...
    def synthesis(self, ws, c,
                  ...
                  ws_bcg=None, cache_backbone=False,
        ...
        planes = planes.view(len(
            planes), 3, 32 * self.rendering_kwargs['triplane_depth'],
            planes.shape[-2], planes.shape[-1])

        # Perform volume rendering
        feature_samples, depth_samples, weights_samples = self.renderer(
            planes, self.decoder, ray_origins,
        ...
        if self.decoder.activation == "sigmoid":
            # Scale to (-1, 1), taken from ray marcher
            feature_image = feature_image * 2 - 1
        # Generate Background
        if self.bcg_synthesis:
            ws_bcg = ws[:, :self.bcg_synthesis.num_ws] if \
                ws_bcg is None else ws_bcg[:, :self.bcg_synthesis.num_ws]
            if ws_bcg.size(1) < self.bcg_synthesis.num_ws:
                ws_bcg = torch.cat(
                    [ws_bcg, ws_bcg[:, -1:].repeat(
                        1, self.bcg_synthesis.num_ws-ws_bcg.size(1), 1)], 1)
            bcg_image = self.bcg_synthesis(
                ws_bcg, update_emas=update_emas, **synthesis_kwargs)
            bcg_image = torch.nn.functional.interpolate(
                bcg_image, size=feature_image.shape[2:],
        ...
        if self.torgb:
            rgb_image = self.torgb(
                feature_image, ws[:, -1], fused_modconv=False)
            rgb_image = rgb_image.to(
    ...
    def sample(self,
               coordinates,
               directions,
               z, c, truncation_psi=1,
               truncation_cutoff=None,
               update_emas=False, **synthesis_kwargs):
        ws = self.mapping(
            z, c, truncation_psi=truncation_psi,
            truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        planes = self.backbone.synthesis(
            ws, update_emas=update_emas, **synthesis_kwargs)
        planes = planes.view(len(
            planes), 3, 32 * self.rendering_kwargs['triplane_depth'],
            planes.shape[-2], planes.shape[-1])
        return self.renderer.run_model(planes, self.decoder,
                                       coordinates, directions,
                                       self.rendering_kwargs)

    def sample_mixed(self, coordinates, directions, ws,
    ...
    def forward(self, z, c, truncation_psi=1,
                truncation_cutoff=None, neural_rendering_resolution=None,
        ...
        c, delta_c = self.apply_delta_c(
            z, c, truncation_psi=truncation_psi,
            truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        return self.synthesis(
            ws, c, update_emas=update_emas,
...
class OSGDecoder(torch.nn.Module):
    def __init__(self, n_features, options):
        ...
        self.net = torch.nn.Sequential(
            ...
            torch.nn.Softplus(),
            FullyConnectedLayer(
                self.hidden_dim, 1 + options['decoder_output_dim'],
                lr_multiplier=options['decoder_lr_mul'])
        )
    ...
    def forward(self, sampled_features, ray_directions):
        ...
        if self.activation == "sigmoid":
            ...
            rgb = torch.nn.functional.leaky_relu(
                rgb, 0.2, inplace=True) * math.sqrt(2)
